...
<template>
	<div class="p-6 space-y-6">
		...
		<UTabs :items="tabs"
				...
				<div class="space-y-4 mt-4">
					<div v-if="!projects.length"
						...
						<UIcon name="i-heroicons-cube"
							class="text-5xl mb-3" />
						<p class="font-medium">Sin proyectos</p>
						<UButton class="mt-4"
							@click="modals.deploy = true">Desplegar proyecto</UButton>
					</div>

					<UCard v-for="proj in projects"
						:key="proj.name">
						<!-- Header del proyecto -->
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-3">
								...
								<div>
									<p class="font-semibold">{{ proj.name }}</p>
									<p class="text-xs text-gray-400 font-mono">{{ proj.path }}</p>
								</div>
							</div>

							<div class="flex items-center gap-3">
								<UBadge :color="proj.running > 0 ? 'success' : 'neutral'"
									variant="subtle">
						...
						<div v-if="proj.containers.length"
							class="mt-4 space-y-2">
							<div v-for="c in proj.containers"
								:key="c.id"
								...
								<div class="flex items-center gap-3">
									...
									<UBadge :color="stateBadgeColor(c.state) as ButtonProps['color']"
										variant="subtle"
										size="sm">
										{{ c.state }}
									</UBadge>
									<ContainerActions v-bind="c"
										@refresh="loadAll" />
								</div>
							</div>
						</div>
				...
				<div class="mt-4">
					...
					<div class="flex gap-2 mb-4">
						<UInput v-model="search"
							...
							class="max-w-xs" />
						<USelectMenu v-model="stateFilter"
							:options="['all', 'running', 'exited', 'paused']"
							class="w-36" />
					</div>

					<UTable :data="filteredContainers"
				...
				<div class="mt-4 space-y-3">
					...
					<div v-for="s in stats"
						...
						<div>
							...
							<UProgress :modelValue="parseFloat(s.cpuPercent)"
						...
						<div>
							...
							<UProgress :modelValue="parseFloat(s.memPercent)"
			...
			<template #images>
				<div class="mt-4 space-y-4">
					<!-- Pull imagen -->
					<div class="flex gap-2">
						<UInput v-model="pullImage"
							placeholder="nginx:latest"
							icon="i-heroicons-arrow-down-tray"
							class="max-w-sm" />
						<UButton :loading="pulling"
							@click="doPullImage">
							Pull
						</UButton>
						<UButton color="error"
		...
		<!-- ─── Modal Deploy ─── -->
		<UModal v-model:open="modals.deploy"
			:ui="{ content: 'max-w-2xl' }">
			<template #content>

				<UCard>
					<template #header>
						<h3 class="font-semibold text-lg">Desplegar proyecto</h3>
					</template>

					<div class="space-y-4">
						<UFormField label="Nombre del proyecto"
							required>
							<UInput v-model="deployForm.name"
								placeholder="App" />
						</UFormField>

						<UFormField label="docker-compose.yml"
							required>
		...
		<!-- ─── Modal Compose ─── -->
		<UModal v-model:open="modals.compose"
			:ui="{ content: 'max-w-2xl' }">
			<template #content>
				<UCard>
					<template #header>
						<h3 class="font-semibold">docker-compose.yml — {{ selectedProject?.name }}</h3>
					</template>
					<pre class="bg-gray-950 text-green-300 font-mono text-xs rounded p-4 overflow-auto max-h-96">{{
						selectedProject?.compose }}</pre>
					<template #footer>
						<UButton color="neutral"
							@click="modals.compose = false">Cerrar</UButton>
					</template>
				</UCard>
			</template>
		</UModal>
		...
		<!-- ─── Modal Logs ─── -->
		<UModal v-model:open="modals.logs"
			:ui="{ content: 'max-w-3xl' }">
			<template #content>
				<UCard>
					<template #header>
						<h3 class="font-semibold">Logs — {{ logsProject }}</h3>
					</template>
					<div ref="logsRef"
					...
					<template #footer>
						<UButton color="neutral"
							@click="modals.logs = false">Cerrar</UButton>
					</template>
				</UCard>
			</template>
		</UModal>
