	...
	<div class="p-6 space-y-6">
		...
		>
			...
			<p class="text-sm mt-1">
				Crea tu primer subdominio para empezar
			</p>
			<UButton
				class="mt-4 my-2"
				@click="openCreate()"
		...
		>
			...
			>
				...
				<div class="absolute top-3 right-3 flex gap-2">
					<UBadge
						:color="sub.hasSSL ? 'success' : 'warning'"
						variant="subtle"
						size="xs"
					>
						<UIcon
							:name="sub.hasSSL ? 'i-heroicons-lock-closed' : 'i-heroicons-lock-open'"
							class="mr-1"
						/>
						{{ sub.hasSSL ? 'SSL' : 'HTTP' }}
					</UBadge>
					<UBadge
						:color="sub.enabled ? 'success' : 'neutral'"
						variant="subtle"
						size="xs"
					>
		...
		<UModal v-model:open="modals.create">
			<template #content>
				<UCard>
					...
					<div class="space-y-4">
						<UFormField
							label="Nombre del sitio (identificador)"
							required
						>
							<UInput
								v-model="form.name"
						...
						<p
							v-if="form.subdomain && form.domain"
