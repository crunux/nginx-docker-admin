...
const { systemStats, connected, connect, disconnect, subscribeStats } = useLogStream()
		...
		<template #body>
			<UPageGrid>
				...
				>
					<template #footer>
						<div
							v-if="card.title === 'Memory'"
							class="w-full text-2xl font-bold"
						>
							<UProgress
								:model-value="systemStats?.ram ? parseFloat(`${systemStats.ram.percent}`) : 0"
								size="lg"
						...
						<div
							v-else-if="card.title === 'CPU'"
							class="w-full text-2xl font-bold"
						>
							<UProgress
								:model-value="systemStats?.cpu ? parseFloat(`${systemStats.cpu.usage}`) : 0"
								size="lg"
						...
						<div
							v-else-if="card.title === 'Disk'"
							class="w-full text-2xl font-bold"
						>
							<UProgress
								class="w-full"
								:model-value="systemStats?.disk ? parseFloat(`${systemStats.disk[0]?.percent}`) : 0"
								size="lg"
